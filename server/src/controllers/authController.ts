...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign({ adminId: admin.id, type: 'direct-reset' }, process.env.JWT_SECRET!, { expiresIn: '5m' }); // 5 minutes validity
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!decoded || decoded.type !== 'direct-reset') return res.status(403).json({ error: 'Invalid or expired token' });
